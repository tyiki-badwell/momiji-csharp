...
namespace Momiji.Interop.Vst
{
    ...
    namespace AudioMaster
    {
        ...
        internal delegate nint CallBack(
            [In] nint/*AEffect^*/		effect,
            [In] Opcodes opcode,
            [In] int index,
            [In] nint value,
            [In] nint ptr,
    ...
    internal unsafe struct AEffect
    {
        ...
        public nint resvd1;      //< reserved for Host, must be 0
        public nint resvd2;      //< reserved for Host, must be 0
        ...
        public nint _object;          //< #AudioEffect class pointer
        public nint user;            //< user-defined pointer
        ...
        [UnmanagedFunctionPointer(CallingConvention.Cdecl, CharSet = CharSet.Ansi, SetLastError = false)]
        internal delegate nint VSTPluginMain(
            //[In][MarshalAs(UnmanagedType.FunctionPtr)]AudioMaster.CallBack audioMaster
            [In] nint/*AudioMaster.CallBack*/ audioMaster
        );

        [UnmanagedFunctionPointer(CallingConvention.Cdecl, CharSet = CharSet.Ansi, SetLastError = false)]
        internal delegate nint DispatcherProc(
            [In] nint/*AEffect^*/		effect,
            [In] Opcodes opcode,
            [In] int index,
            [In] nint value,
    ...
    internal struct VstMidiSysexEvent
    {
        ...
        public nint resvd1;       ///< zero (Reserved for future use)
    ...
    internal struct VstEvents
    {
        ...
        public nint reserved;        //< zero (Reserved for future use)
                                        //nint	events;			//VstEvent* events[2];	///< event pointer array, variable size
                                        //-------------------------------------------------------------------------------------------------------
    };
    ...
    */
    ...
    {
        //-------------------------------------------------------------------------------------------------------
        public nint inputs;                      //< float** input audio buffers
        public nint outputs;                 //< float** output audio buffers
        public int numSamplesInput;          //< number of incoming samples
        public int numSamplesOutput;         //< number of outgoing samples
        public nint numSamplesInputProcessed;    //< Int32* number of samples actually processed of input
        public nint numSamplesOutputProcessed;   //< Int32* number of samples actually processed of output
                                                    //-------------------------------------------------------------------------------------------------------
    };

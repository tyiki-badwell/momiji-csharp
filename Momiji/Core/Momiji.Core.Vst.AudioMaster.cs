...
public class AudioMaster<T> : IDisposable where T : struct
{
    ...
    internal IDictionary<nint, Effect<T>> EffectMap { get; } = new ConcurrentDictionary<nint, Effect<T>>();
    ...
    internal nint AudioMasterCallBackProc(
        nint/*AEffect^*/		aeffectPtr,
        Opcodes opcode,
        int index,
        nint value,
        nint ptr,
    ...
    )
    {
        ...
        switch (opcode)
        {
            ...
            case Opcodes.audioMasterGetSampleRate:
                {
                    return new nint(SamplingRate);
                }
            case Opcodes.audioMasterGetBlockSize:
                {
                    return new nint(BlockSize);
                }
            case Opcodes.audioMasterGetCurrentProcessLevel:
                {
                    var p = _param.AsSpan(0, 1);
                    return new nint((int)p[0].vstProcessLevels);
                }

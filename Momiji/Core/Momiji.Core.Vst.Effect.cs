...
public class VstBuffer<T> : IDisposable where T : struct
{
    ...
    internal PinnedBuffer<nint[]> Buffer { get; }
...
public class VstBuffer2<T> : IDisposable where T : struct
{
    ...
    public nint GetChannelBuffer(int channel) => Buffer.Target[channel];
}
...
internal class Effect<T> : IEffect<T>, IDisposable where T : struct
{
    ...
    private readonly nint _aeffectPtr;
    ...
    private static readonly int SIZE_OF_INTPTR = Marshal.SizeOf<nint>();
    ...
    private void Open()
    {
        ...
        Dispatcher(
            ...
            default,
            new nint(_audioMaster.BlockSize),
            default,
            default
        );
        ...
        Dispatcher(
            ...
            default,
            new nint((int)((SIZE_OF_T == 8) ? VstProcessPrecision.kVstProcessPrecision64 : VstProcessPrecision.kVstProcessPrecision32)),
            default,
            default
        );
    ...
    public IWindow OpenEditor()
    {
        ...
        try
        {
            using var rectPtrBuffer = new PinnedBuffer<nint>(new nint());
    ...
    private void OnPostPaint(nint hWnd)
    {
        /*

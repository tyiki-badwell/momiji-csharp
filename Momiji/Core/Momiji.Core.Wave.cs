...
public class WaveOut<T> : IDisposable where T : struct
{
    ...
    private readonly TransformBlock<nint, PcmBuffer<T>> _releaseAction;

    private readonly IDictionary<nint, WaveHeaderBuffer> _headerBusyPool = new ConcurrentDictionary<nint, WaveHeaderBuffer>();
    private readonly IDictionary<nint, PcmBuffer<T>> _dataBusyPool = new ConcurrentDictionary<nint, PcmBuffer<T>>();
    ...
    )
    {
        ...
        _releaseAction = new TransformBlock<nint, PcmBuffer<T>>(headerPtr => Unprepare(headerPtr));
    ...
    private nint Prepare(PcmBuffer<T> source, CancellationToken ct)
    {
        var header = _headerPool.Receive(ct);
        {
            ...
            waveHeader.user = nint.Zero;
            waveHeader.next = nint.Zero;
            waveHeader.reserved = nint.Zero;
        }
    ...
    private PcmBuffer<T> Unprepare(nint headerPtr)
    {
    ...
    private void Send(nint headerPtr)
    {
        var mmResult =

...
public static class SFrameBSInfoBufferExtensions
{
    private static readonly List<FieldInfo> _layerInfoList = InitializeLayerInfoList();

    private static List<FieldInfo> InitializeLayerInfoList()
    {
        var temp = new List<FieldInfo>();
        for (var idx = 0; idx < 128; idx++)
        {
            var item = typeof(SFrameBSInfo).GetField($"sLayerInfo{idx:000}") ?? throw new H264Exception("InitializeLayerInfoList failed.");
            temp.Add(item);
        }
        return temp;
    }
    ...
    public static SLayerBSInfo SLayerInfo(this SFrameBSInfoBuffer self, int index)
    {
        ...
        var value = _layerInfoList[index].GetValue(self.Target) ?? throw new H264Exception("SLayerInfo failed.");
...
public class H264Encoder : IDisposable
{
    ...
    private static unsafe void CopyMemory(
        nint Destination,
        int maxLength,
        nint Source,
        int Length)
